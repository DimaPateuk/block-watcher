...
import { MockRpcService, MockRpcBlock } from '../../test/utils/rpc-mock';

// Helper function to convert block generator output to MockRpcBlock format
function toMockRpcBlocks(blocks: ReturnType<typeof createBlockRange>): MockRpcBlock[] {
...
describe('EvmWatcherProvider E2E', () => {
  ...
  describe('Integration Scenarios - Mixed Database States', () => {
    beforeEach(async () => {
      ...
      const missingBlocks = createBlockRange(206n, 209n, {
        chainId: 12,
        salt: 'integration-missing',
      });
      mockRpc.setBlocks(12, toMockRpcBlocks(missingBlocks));
    });
  ...
  describe('Performance and Stress Tests', () => {
    beforeEach(async () => {
      ...
      const missingRange1 = createBlockRange(3011n, 3049n, {
        chainId: 99,
        salt: 'perf-missing-1',
      });
      const missingRange2 = createBlockRange(3061n, 3099n, {
        chainId: 99,
        salt: 'perf-missing-2',
      });
      
      // Convert to MockRpcBlock format and combine
      const allMissingBlocks = [
        ...toMockRpcBlocks(missingRange1),
        ...toMockRpcBlocks(missingRange2)
      ];
      
      mockRpc.setBlocks(99, allMissingBlocks);
    });
    ...
    it('should handle large number of missing blocks efficiently', async () => {
      ...
      expect(block3011).not.toBeNull();
      expect(block3015).not.toBeNull();
      expect(block3011?.number).toBe(3011n);
      expect(block3015?.number).toBe(3015n);
      // Hashes are deterministically generated by the utility, so just verify they exist
      expect(block3011?.hash).toMatch(/^0x[a-f0-9]{64}$/);
      expect(block3015?.hash).toMatch(/^0x[a-f0-9]{64}$/);
